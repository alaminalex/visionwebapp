...
require_once("include/rejected_jobfile2_variables.php");
...
$page_layouts["rejected_jobfile2_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["jobfile_id"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["jobfile_id"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
