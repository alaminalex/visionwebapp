...
$page_layouts["Top_Designer_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=Top_Designer.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=Top_Designer.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
$xt->assign("designer2_id_fieldheader", true);
$xt->assign("files_fieldheader", true);
$xt->assign("taken_time_avg_fieldheader", true);
...
if($mastertable == "group")
{
	...
	$dparams = array("detailtable" => "Top_Designer", "keys" => $masterkeys);
	$master = array(); 
	AssignFunction($master, "DisplayMasterTableInfoForPrint_group", $dparams);
...
{
	require_once("include/department_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$dparams = array("detailtable" => "Top_Designer", "keys" => $masterkeys);
	$master = array(); 
	AssignFunction($master, "DisplayMasterTableInfoForPrint_department", $dparams);
...
if($mastertable == "subdivision")
{
	...
	$dparams = array("detailtable" => "Top_Designer", "keys" => $masterkeys);
	$master = array(); 
	AssignFunction($master, "DisplayMasterTableInfoForPrint_subdivision", $dparams);
