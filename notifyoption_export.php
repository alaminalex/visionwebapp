...
$layout = new TLayout("export", "Coral1vApplication", "MobilevApplication");
$layout->version = 2;
...
$page_layouts["notifyoption_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["notify_option_id"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["notify_option_id"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
