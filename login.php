...
require_once('classes/loginpage_ad.php');
add_nocache_headers();




$layout = new TLayout("login11", "Coral1vApplication", "MobilevApplication");
$layout->version = 2;
...
$pageObject = new LoginPage_AD( $params );
$pageObject->init();

$adSubmit = false;
if (GetGlobalData("ADSingleSign",0) && $_SERVER["REMOTE_USER"]!='')
{
	$pUsername = GetGlobalData("ADLogin",'');
	$pPassword = GetGlobalData("ADPassword",'');
	$adSubmit = true;
}
if (!isset($pUsername))
{
...
if ((@$_POST["btnSubmit"] == "Login" || $adSubmit) && $logacc)
{
	...
	if ($retval)
	{
		...
		if ($d) 
		{
			...
			{
				$dummy = array();
				$globalEvents->AfterSuccessfulLogin($pUsername, $pPassword, $dummy, $pageObject);
			}
			if ($onFly == 2) 
			{
				if($myurl) 
