<?php 
class eventclass_Download_Job  extends eventsBase
{ 
	function eventclass_Download_Job()
	{
	// fill list of events

//	onscreen events
		$this->events["Download_Job_snippet"] = true;

	}

//	handlers
//	onscreen events
function Download_Job_snippet(&$params)
{
...
if (isset($_GET["work_id"]) && isset($_GET["job_id"]) && $_GET["work_id"] != "" && $_GET["job_id"] != "")
	...
	$select_job_sql = "
			...
			job.jobstatus_id = jobstatus.jobstatus_id AND `work_id`=$selected_work_id
			INNER JOIN pdf ON
			job.pdf_id = pdf.pdf_id
			";
	...
	if ($number_of_jobs == 1)
     ...
     echo '<form id="files" method="post" action="/'.$_SESSION["my_server_share"].'/download_job_list.php'.$load_urloptions.'">';
		...
		if(!$update_job_query = mysqli_query($mysqlilink, $update_job_sql))
			{
				...
				header("location: /".$_SESSION["my_server_share"]."/pending_jobs_list.php?action=downloaded");
				} // IF JOBSTATUS IS ACCEPTED, SET STATUS TO DOWNLOADING ELSE MOVE TO DOWNLOAD PROCESS
					...
					if (isset($_POST["user_input_ok"]))
						{
							...
							$select_file_sql = "SELECT `file_url`, `work_id` from jobfile WHERE `file_url`='$safe_selected_file' AND `work_id`='$selected_work_id' ";
							...
							if ($is_duplicate > 0)
								...
								//PROCESS THE JOB INSERT THE INFORMATION INTO THE DATABASE
									...
									$insert_new_file_sql = "
									INSERT INTO `visionwebapp`.`jobfile` (
									`jobfile_id`, `file_url`, `client_id`, `work_id`, `job_id`, `project_id`, `folder_name`, `filename`, `jobfile_type`, `jobfile_note`, `jobfile_status_id`, `downloader_id`,  `download_date`, `distribute_date`, 	`start_date`, `end_date`, `qcstart_date`, 	`qcend_date`, `upload_date`, `notify_date`, `reject_date`, `reject_reason`) VALUES (
									NULL,		 '$file_url', '$client_id', '$work_id', '$job_id', '$project_id', '$folder_name', '$filename', '$jobfile_type', '$jobfile_note', '$jobfile_status_id', '$downloader_id', '$download_date', '$distribute_date', '$start_date', '$end_date', '$qcstart_date', '$qcend_date', '$upload_date', '$notify_date', '$reject_date', '$reject_reason');";
