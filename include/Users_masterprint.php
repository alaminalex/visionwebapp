...
function DisplayMasterTableInfoForPrint_Users($params)
{
	...
	$xt->assign("lastaccess_mastervalue", $viewControls->showDBValue("lastaccess", $data, $keylink));
	$format = $settings->getViewFormat("lastaccess");
	$class = " rnr-field-text";
	if($format == FORMAT_FILE) 
		$class = ' rnr-field-file'; 
	if($format == FORMAT_AUDIO)
	...
	if($format == FORMAT_CHECKBOX)
		$class = ' rnr-field-checkbox';
	if($format == FORMAT_NUMBER || IsNumberType($settings->getFieldType("lastaccess")))
		$class = ' rnr-field-number';
		
	$xt->assign("lastaccess_class", $class); // add class for field header as field value
