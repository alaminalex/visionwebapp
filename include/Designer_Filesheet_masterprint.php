...
function DisplayMasterTableInfoForPrint_Designer_Filesheet($params)
{
	global $cman;
	...
	if( $detailtable == "Pending Jobs" )
	{
		$keysAssoc["work_id"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("work_id", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("work_id", $keys[1-1], "", true);
	...
	$layout = GetPageLayout("Designer_Filesheet", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("Designer_Filesheet", "masterprint"));
}
