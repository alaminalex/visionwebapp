<?php 
class eventclass_Designer_Level2_Filesheet  extends eventsBase
{ 
	function eventclass_Designer_Level2_Filesheet()
	{
	// fill list of events
...
//	onscreen events
		$this->events["Designer_Filesheet_snippet1"] = true;
...
$strWhereClause = whereAdd($strWhereClause, "jobfile.designer2_id = '".$_SESSION["user_id"]."' AND `jobfile`.`job_id`='".$GLOBALS["job_id"]."'");

}
else
{
//$strWhereClause = "`designer2_id`='".$_SESSION["user_id"]."'";
$strWhereClause = whereAdd($strWhereClause, "jobfile.designer2_id = '".$_SESSION["user_id"]."'");
...
//	onscreen events
function Designer_Filesheet_snippet1(&$params)
{
/*
		$get_query_string = $_GET["q"];
$string = $get_query_string;
