...
function DisplayMasterTableInfoForPrint_File_Distribution($params)
{
	global $cman;
	...
	if( $detailtable == "jobfile" )
	{
		$keysAssoc["jobfile_id"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("jobfile_id", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("jobfile_id", $keys[1-1], "", true);
	...
	$layout = GetPageLayout("File_Distribution", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("File_Distribution", "masterprint"));
}
