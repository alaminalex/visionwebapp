<?php 
class eventclass_File_Distribution_Rejected  extends eventsBase
{ 
	function eventclass_File_Distribution_Rejected()
	{
	// fill list of events
...
//	onscreen events
		$this->events["File_Distribution_snippet24"] = true;
		$this->events["File_Distribution_snippet33"] = true;
		$this->events["File_Distribution_snippet6"] = true;
...
//	onscreen events
function File_Distribution_snippet24(&$params)
{
echo '<span style="font-size:12px; font-weight:bold; color: blue;">WORK ID: '.$GLOBALS["work_id"].' | JOB ID: '.$GLOBALS["job_id"].'</span> | ';
echo '<a href="file_distribution_rejected_list.php?q=(work_id~equals~'.$GLOBALS["work_id"].')(job_id~equals~'.$GLOBALS["job_id"].')">Show All Files</a> | ';
echo '<a href="file_distribution_rejected_list.php?q=(jobfile_status_id~equals~3)(work_id~equals~'.$GLOBALS["work_id"].')(job_id~equals~'.$GLOBALS["job_id"].')">Need Distribute Files</a> | ';
echo '<a href="file_distribution_rejected_list.php?q=(jobfile_status_id~equals~14)(work_id~equals~'.$GLOBALS["work_id"].')(job_id~equals~'.$GLOBALS["job_id"].')">Need to QC</a>';
;
}
function File_Distribution_snippet33(&$params)
{
$file_list_get = $_GET["file_list"];
$file_list_get = explode(",", $file_list_get);
...
function File_Distribution_snippet6(&$params)
{
$file_list_get = $_GET["file_list"];
$file_list_get = explode(",", $file_list_get);
echo COUNT($file_list_get)." Files are selected";
