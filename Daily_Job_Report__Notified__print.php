...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=Daily_Job_Report__Notified_.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=Daily_Job_Report__Notified_.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
$xt->assign("notify_date_fieldheader", true);
...
if($mastertable == "notifyoption")
{
	...
	$dparams = array("detailtable" => "Daily Job Report (Notified)", "keys" => $masterkeys);
