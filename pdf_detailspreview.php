...
$page_layouts["pdf_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	price - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("price", $data, $keylink);
			$row["price_value"] = $value;
			$format = $pSet->getViewFormat("price");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("price")))
				$class = ' rnr-field-number';
			$row["price_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("pdf", "detailspreview"));
