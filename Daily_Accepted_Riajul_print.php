...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=Daily_Accepted_Riajul.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=Daily_Accepted_Riajul.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
{
	require_once("include/clients_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$dparams = array("detailtable" => "Daily Accepted Riajul", "keys" => $masterkeys);
	
	$xt->assign_function("showmasterfile", "DisplayMasterTableInfoForPrint_clients", $dparams);
