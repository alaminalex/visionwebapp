...
$page_layouts["job_detailspreview"] = $layout;
...
if($mastertable == "Client Instruction")
{
	$where = "";
		$formattedValue = make_db_value("pdf_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
...
if($rowcount) 
{
	...
	//	job_directory - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("job_directory", $data, $keylink);
			$row["job_directory_value"] = $value;
			$format = $pSet->getViewFormat("job_directory");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("job_directory")))
				$class = ' rnr-field-number';
			$row["job_directory_class"] = $class;
	//	complexity1 - 
			$viewContainer->recId = $recordsCounter;
	...
	//	finish_date - Datetime
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("finish_date", $data, $keylink);
			$row["finish_date_value"] = $value;
			$format = $pSet->getViewFormat("finish_date");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("finish_date")))
				$class = ' rnr-field-number';
			$row["finish_date_class"] = $class;
	//	uploader_id - Custom
			$viewContainer->recId = $recordsCounter;
	...
	//	activity_log - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("activity_log", $data, $keylink);
			$row["activity_log_value"] = $value;
			$format = $pSet->getViewFormat("activity_log");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("activity_log")))
				$class = ' rnr-field-number';
			$row["activity_log_class"] = $class;
	//	time_remaining - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("time_remaining", $data, $keylink);
			$row["time_remaining_value"] = $value;
			$format = $pSet->getViewFormat("time_remaining");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("time_remaining")))
				$class = ' rnr-field-number';
			$row["time_remaining_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("job", "detailspreview"));
